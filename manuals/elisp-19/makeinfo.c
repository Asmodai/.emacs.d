...

/* This is Makeinfo version 1.65.  If you change the version number of
   Makeinfo, please change it here and at the lines reading:

    int major_version = 1;
    int minor_version = 65;

   in the code below.

   Makeinfo is authored by Brian Fox (bfox@gnu.ai.mit.edu). */
...
void
  cm_asterisk (), cm_dots (), cm_enddots (), cm_bullet (), cm_TeX (),
  cm_copyright (), cm_code (), cm_samp (), cm_file (), cm_kbd (), cm_url (),
  cm_email (), cm_key (), cm_ctrl (), cm_var (), cm_dfn (), cm_emph (),
  cm_strong (), cm_cite (), cm_italic (), cm_bold (), cm_roman (),
  cm_title (), cm_w (), cm_refill (), cm_titlefont ();
...
void
  ...
  cm_cartouche (), cm_ignore_sentence_ender (), cm_detailmenu (),
  cm_dircategory (), cm_direntry ();
...
print_version_info ()
{
  printf ("This is GNU Makeinfo version %d.%d, from texinfo-3.8.\n",
...
enum insertion_type { menu, detailmenu, quotation, lisp, smalllisp, example,
  ...
  deftypemethod, deftp, direntry, bad_type };

char *insertion_type_names[] = { "menu", "detailmenu", "quotation", "lisp",
    ...
    int implicit_code;
{
  ...
  result = (unsigned char *) xmalloc (1 + length);
...
{
  char *str1 = (*element1)->entry;
  char *str2 = (*element2)->entry;
  ...
  while (temp != (INDEX_ELT *) NULL)
    {
      ...
      /* Maybe should set line number to the defining_line?  */
  ...
  for (item = 0; (index = array[item]); item++)
    {
	 ...
	 at the line which contains the entry. */
      ...
      /* Print the entry.  We've already expanded any commands,
         including any implicit @code.  */
	...
	indent (40 - output_column);
      ...
      /* A pathological document might have index entry outside of any
         node.  Don't crash.  Perhaps should warn.  */
      execute_string ("%s.\n", index->node ? index->node : "(none)");
