...
cm_print (), cm_error (), cm_point (), cm_smallbook (), cm_headings (),
cm_today ();
...
/* The `def' commands. */
  ...
  {"defvr", cm_defun, false},
  {"defvrx", cm_defun, false},
  {"defvar", cm_defun, false},
  {"defvarx", cm_defun, false},
  {"defopt", cm_defun, false},
  {"defoptx", cm_defun, false},
  {"deftypefn", cm_defun, false},
  {"deftypefnx", cm_defun, false},
  {"deftypefun", cm_defun, false},
  {"deftypefunx", cm_defun, false},
  {"deftypevr", cm_defun, false},
  {"deftypevrx", cm_defun, false},
  {"deftypevar", cm_defun, false},
  {"deftypevarx", cm_defun, false},
  {"defcv", cm_defun, false},
  {"defcvx", cm_defun, false},
  {"defivar", cm_defun, false},
  {"defivarx", cm_defun, false},
  {"defop", cm_defun, false},
  {"defopx", cm_defun, false},
  {"defmethod", cm_defun, false},
  {"defmethodx", cm_defun, false},
  {"deftp", cm_defun, false},
  {"deftpx", cm_defun, false},
...
struct option long_options[] =
{
  ...
  { "no-pointer-validate", 0, &validating, false }, /* formerly -nv */
...
`+footnote-style STYLE' to set the footnote style to STYLE.  STYLE should\n\
  either be `MN' for `make node', or `EN' for `end node'.\n\n",
...
enum insertion_type
{
  ...
  deffn, defun, defmac, defspec, defvr, defvar, defopt,
  deftypefn, deftypefun, deftypevr, deftypevar, defcv,
  defivar, defop, defmethod, deftp,
  bad_type
};
...
/* Here is a structure which associates sectioning commands with
   ...
   section.  Because of the way that matching is done, you must
   put the longest version of substrings first.  That is,
   unnumberedsub must precede unnumbered. */
struct {
  char *name;
  int level;
} section_alist[] = {
...
copy_substring (start, end)
     ...
     char * end;
{
  char * result = ((char *) (xmalloc ((end - start) + 1)));
  char * scan = start;
...
{
  register int c;
  register char * scan_string = ((*string_pointer) + 1);
     ...
     char *string;
{
  ...
  while ((*scan_string) != '\0')
      ...
      else
	{
	  ...
	  while (1)
	    {
	      register int c = (*scan_string++);

	      if (!c ||
		  (!brace_level &&
		   (whitespace (c) ||
		    DEFUN_SELF_DELIMITING (c) ||
		    ((!allow_brace_gathering) && c == '{') ||
		    (c == '}'))))
		{
		  scan_string--;
		  break;
		 ...
		 then that command (and it's associated braces) are
		 part of the token that we gathering. */
	      if (c == COMMAND_PREFIX && !self_delimiting (*scan_string))
		{
		  allow_brace_gathering++;
		  continue;
		}
	      
	      if (allow_brace_gathering)
		{
		  if (c == '{')
		    brace_level++;
		  else
...
EN   2) Make them appear at the bottom of the node that they originally
        appeared in.
*/
