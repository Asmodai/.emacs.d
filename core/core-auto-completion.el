...
MODE must match the symbol passed in `bootstrap|defvar-company-backends'.
The initialization function is hooked to `MODE-hook'."
  (let ((mode-hook (intern (format "%S-hook" mode)))
    ...
    `(when (bootstrap-conf-layer/package-usedp 'company)
       (defun ,func ()
         ,(format "Initialize company for %S" mode)
         (when auto-completion-enable-snippets-in-popup
           (setq ,backend-list (mapcar 'bootstrap//show-snippets-in-company
                                       ,backend-list)))
...
The initialization function is hooked to `MODE-hook'."
  (let ((mode-hook (intern (format "%S-hook" mode)))
    ...
    `(when (bootstrap-conf-layer/package-usedp 'auto-complete)
       (defun ,func ()
